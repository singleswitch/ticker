...
class GridSimulation():
    ...
    def init(self, i_scan_delay, i_click_time_delay, i_config=None):
    ...
    def compute(self, i_sentence, i_params):
        ...
        self.init(scan_delay, click_time_delay, i_config=None)
        print "****************************************************************************************************************************"
    ...
    def probResults(self, i_grnd_truth_word, i_params):
        ...
        (states, transitions, transition_probs, scan_delays ) = self.getStates(i_grnd_truth_word, i_params, self.letter_info)
    ...
    def probsToResults(self, i_grnd_truth_word, i_scan_probs, i_click_probs, i_err_probs, i_wpm_probs):
        (scans,scans_wpm, clicks, errors) = self.getPossibleResultValues(i_grnd_truth_word, 
            len(i_scan_probs), len(i_click_probs), len(i_err_probs)) 
    ...
    def stateProbs(self, i_states, i_transitions, i_transition_probs,  i_scan_delays, i_T, i_grnd_truth_word, i_max_spurious, i_display):
        ...
        * o_err_probs: Probs of states associated with column and "_", but not correct, i.e., an erroneous word selection
        ...
        scan_probs[0,(i_scan_delays[0]-1)] = 0.0
        ...
        #scan_probs = scan_probs[:(max_scans*cur_time + 1)]
        ...
        #scan_probs2 = np.exp(scan_probs2)
    ...
    def updateMaxTimeFailureProbs(self, state_probs, scan_probs, click_probs, err_probs, M, N, i_T, i_bail_out_time):
        ...
        print "Including failure probabilities"
    ...
    def getStates(self,i_word, i_params, i_letter_info):
        """States in format: 
        ...
        for letter_idx in range(0, n_input_letters):
            ...
            for click in range(0, n_errors):
                if display :
                    ...
                    print "---------------------------------------"
                (tmp_row_states, tmp_row_transitions, tmp_row_probs) = self.getRowStates( letter, letter_idx, click, i_letter_info['0'] , display )
                ...
                for undo in range(0, n_undo):
                    (tmp_col_states, tmp_col_transitions, tmp_col_probs) = self.getColStates( letter, letter_idx, click, 
                        undo,  n_undo,  n_errors, correct_id,i_letter_info['1'], display ) 
        ...
        if display:
           self.displayStates( row_states, col_states, row_transitions, col_transitions, n_undo, n_errors, n_input_letters)
    ...
    def getRowStates(self, i_input_letter, i_input_letter_num, i_click, i_letter_info, i_display):
        ...
        for r in range(0 , len(row_ids)-1):
            ...
            (prob_click,  prob_miss )  = self.clickProb( row_ids,  i_letter_info,  gauss_mean, r ,  i_display )
        ...
        for r in range(0 , len(row_ids)):
            ...
            for c in range(0, len(dest_list)):
                ...
                if (dest_list[c] == i_input_letter) and (i_click == 0):
                    ...
                    #print "r,c = ", r, " ", c, " letter = ", i_input_letter, " undo = ", i_undo, " correct_id = ", i_correct_id
    ...
    def displayStates(self, i_row_states, i_col_states, i_row_transitions, i_col_transitions, i_n_undo, i_n_clicks, i_n_input_letters):
        ...
        for click in range(0, i_n_clicks):
            ...
            disp_str+= "===================================================================="
            print disp_str
            self.__displayStates(i_row_states, i_row_transitions, click, 1, i_n_clicks, n_letters_rows, i_n_input_letters)
            disp_str="---------------------------------------------------------------------------------------"
            disp_str+= "--------------------------------------------------------------------"
            print disp_str
            self.__displayStates(i_col_states, i_col_transitions, click, i_n_undo, i_n_clicks, n_letters_cols, i_n_input_letters)
        print disp_str
            
    def __displayStates(self, i_states, i_transitions, i_click, i_n_undo, i_n_clicks, i_n_output_letters, i_n_input_letters):
        for n in range(0, i_n_output_letters):
            ...
            for m in range(0, i_n_input_letters):
                for undo in range(0, i_n_undo ):
                    ...
                    dest_miss =  i_transitions[state_id][0]
                    dest_click =  i_transitions[state_id][1]
                    disp_str += "%s:[%s %s] " % (state_id, dest_click, dest_miss)
            print disp_str
    ...
    def getLetterDurations(self , i_display=False):
        ...
        for row_cnt in range(0, len(self.config)):
            ...
            durations = self.__getSeqDuration(row, i_display)
            letter_info.append(dict(durations))
        durations = self.__getSeqDuration(self.getGridRow(), i_display)
        o_info = {'1' : list(letter_info), '0': dict(durations) }
        return o_info
      
    def __getSeqDuration(self, i_seq, i_display):
        durations = {}
        ...
        durations['gauss_mean'] =  durations['t_start'] + self.click_time_delay         
        durations['t_start'][0] = 0.0 
        total_time = durations['t_end'][-1]
       
        if i_display:
            print "Input seq=%s " % (''.join(i_seq)),
            print "t_start=" , self.utils.stringVector( durations['t_start'], i_type="%.1f" ),
            print "t_end=" , self.utils.stringVector( durations['t_end'], i_type="%.1f" ),
