...
class GridToTickerSimulationExamples(GridToTickerSimulation):
    ############################################### Init
    def __init__(self):
        GridToTickerSimulation.__init__(self)
    ...
    def initWordExampleParams(self):
        ...
        fp_rate=0.0 
        scan_delay=1.0
        click_time_delay=0.5*scan_delay + 0.1 
        std=0.1
        n_max=10 #Simulate for T=n_max*(n_rows+n_cols)*len(input_word) scans
        draw_tikz=False
        display=True
        self.debug = True
        self.init(i_scan_delay=scan_delay, i_click_time_delay=click_time_delay, i_std=std, i_config=None) 
    ...
    def examples(self):
        self.simpleExample(test_ground_truth=True)
        #self.wordExample("a_", test_ground_truth=True) #standing_", test_ground_truth=True))
        #elf.sentenceExample("at_me_.")        
        p.show()
    ...
    def wordExample(self, i_input_word, test_ground_truth=True) :
        """
        ...
        * Display ball park figures for no noise case 
        ...
        (n_errors, n_undo, fr, fp_rate, scan_delay, click_time_delay, std, n_max, draw_tikz, display) = params
        ...
        if click_probs is None:
            n_clicks = None
        elif np.abs(click_probs) < 1E-6:
            n_clicks = 0
        else:
            n_clicks = len(click_probs) 
    
        (scans,scans_wpm, clicks, errors) = self.getPossibleResultValues(i_input_word,
            len(scan_probs), n_clicks, len(err_probs)) 
 
        self.displayResults(results, True)
        self.displaySpeed(scan_probs, i_input_word)
        if test_ground_truth:
            return results
    ...
    def simpleExample(self, test_ground_truth=False): 
        config=[['a', '_'],['t', 'D']]
        ...
        if test_ground_truth:
            fr = 0.0
            std = 0.000005
            click_time_delay = 0.5- 2*std# scan_delay - 2.0*std
            
        params = (n_errors, n_undo, fr, fp_rate, scan_delay, click_time_delay, std, n_max, draw_tikz, display)
        self.init(scan_delay, click_time_delay, std, config)
        ...
        (states, transitions, transition_probs, scan_delays ) = self.getStates(input_word,  params)
        #Compute the pdfs for the stats
        (scan_probs, click_probs, err_probs, wpm_probs) = self.stateProbs(states, transitions, transition_probs, scan_delays, max_scans,
                    input_word, n_errors, display)
         
        self.displayProbs(scan_probs, click_probs, err_probs ) 
        self.displaySpeed(scan_probs, input_word)
...
if __name__ ==  "__main__":   
    g = GridToTickerSimulationExamples()
    g.examples()
